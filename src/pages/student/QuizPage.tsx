...
const QuizPage = () => {
  ...
  if (isLoading) {
    return (
      <div className="animate-pulse max-w-3xl mx-auto space-y-4">
        ...
        {[1, 2, 3].map(i => <div key={i} className="h-28 bg-background rounded-xl" />)}
      </div>
    );
  }
  ...
  if (submitted && result) {
    ...
    return (
      <div className="max-w-md mx-auto text-center space-y-6 py-10">
        <div className={`w-24 h-24 mx-auto rounded-full flex items-center justify-center ${passed ? 'bg-green-500/20' : 'bg-red-500/20'}`}>
          {passed
        ...
        <div className="bg-card border border-border shadow-lg rounded-2xl p-6 space-y-4">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => navigate(-1)}
            className="flex-1 border border-border text-muted hover:text-white hover:bg-background py-3 rounded-xl font-bold transition-colors"
          >
            رجوع
          </button>
          <button
            onClick={() => { setSubmitted(false); setResult(null); setAnswers({}); }}
            className="flex-1 bg-primary hover:brightness-110 text-white py-3 rounded-xl font-bold transition-colors shadow-lg"
          >
            إعادة الاختبار
          </button>
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      {/* Quiz Header */}
      <div className="bg-card rounded-2xl border border-border shadow-lg p-6 flex items-center justify-between">
        <div>
      ...
      <div className="space-y-4">
        {quiz.questions?.map((question: any, qIdx: number) => (
          <div key={question.id} className="bg-card rounded-2xl border border-border shadow-lg p-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="bg-primary hover:brightness-110 text-white font-bold px-8 py-3 rounded-xl transition-colors disabled:opacity-60 flex items-center gap-2 shadow-lg"
        >
