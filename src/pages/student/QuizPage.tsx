...
const QuizPage = () => {
  ...
  if (!quiz) {
    return <div className="text-center py-20 text-gray-500"><p>لم يتم العثور على الاختبار</p></div>;
  }
  ...
  if (submitted && result) {
    ...
    return (
      <div className="max-w-md mx-auto text-center space-y-6 py-10">
        <div className={`w-24 h-24 mx-auto rounded-full flex items-center justify-center ${passed ? 'bg-emerald-100' : 'bg-red-100'}`}>
          {passed
        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded-2xl p-6 space-y-4">
          <div className="flex justify-between">
            <span className="text-gray-500">نتيجتك</span>
            <span className={`text-2xl font-bold ${passed ? 'text-emerald-600' : 'text-red-500'}`}>
              {Math.round(result.score)}%
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">الإجابات الصحيحة</span>
            <span className="text-gray-900 font-bold">{result.correctCount} / {result.totalQuestions}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">درجة النجاح</span>
            <span className="text-gray-900 font-bold">{quiz.passing_score}%</span>
          </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={() => navigate(-1)}
            className="flex-1 border border-gray-200 text-gray-600 hover:text-gray-900 hover:bg-gray-50 py-3 rounded-xl font-bold transition-colors"
          >
            رجوع
          </button>
          <button
            onClick={() => { setSubmitted(false); setResult(null); setAnswers({}); }}
            className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-xl font-bold transition-colors shadow-sm"
          >
            إعادة الاختبار
          </button>
        </div>
    ...
    <div className="max-w-3xl mx-auto space-y-6">
        ...
        {quiz.questions?.map((question: any, qIdx: number) => (
          <div key={question.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
            <h3 className="font-bold text-gray-900 mb-4">
              <span className="text-indigo-600 ml-2">{qIdx + 1}.</span>
                ...
                <label
                  ...
                  className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${
                    answers[question.id] === answer.id
                      ...
                      : 'border-gray-200 text-gray-700 hover:border-indigo-400 hover:text-gray-900'
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  <div className={`w-4 h-4 rounded-full border-2 flex-shrink-0 ${
                    answers[question.id] === answer.id ? 'border-indigo-600 bg-indigo-600' : 'border-gray-300'
                  }`} />
                  {answer.answer_text}
                </label>
              ))}
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-8 py-3 rounded-xl transition-colors disabled:opacity-60 flex items-center gap-2 shadow-sm"
        >
