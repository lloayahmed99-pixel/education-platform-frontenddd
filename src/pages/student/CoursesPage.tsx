    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full bg-card border border-border rounded-xl px-4 py-2 pr-10 text-white placeholder-muted focus:outline-none focus:border-primary transition-colors shadow-lg"
          />
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="h-72 bg-card rounded-2xl border border-border shadow-lg" />
          ))}
        </div>
              ...
              {Array.from({ length: data.pages }, (_, i) => i + 1).map(p => (
                <button
                  ...
                  className={`w-9 h-9 rounded-lg font-bold transition-colors ${
                    p === page ? 'bg-primary text-white' : 'bg-card border border-border text-muted hover:border-primary hover:text-primary'
                  }`}
                >
                  {p}
                </button>
