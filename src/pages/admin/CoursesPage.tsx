              ...
              {isLoading ? (
                Array.from({ length: 5 }, (_, i) => (
                  <tr key={i} className="border-b border-border animate-pulse">
                    {[1,2,3,4,5].map(j => <td key={j} className="px-5 py-4"><div className="h-4 bg-background rounded" /></td>)}
                  </tr>
                ))
              ) : data?.courses?.map((course: any) => (
                <tr key={course.id} className="border-b border-border hover:bg-background/60 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <span className={`text-xs px-2 py-1 rounded-full font-semibold ${course.published ? 'bg-green-500/20 text-green-400' : 'bg-background text-muted'}`}>
                      {course.published ? 'منشور' : 'مسودة'}
                    </span>
                  </td>
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2">
                      <button onClick={() => publishMutation.mutate({ id: course.id, published: !!course.published })} title={course.published ? 'إخفاء' : 'نشر'} className={course.published ? 'text-amber-500 hover:text-amber-600' : 'text-green-400 hover:text-green-500'}>
