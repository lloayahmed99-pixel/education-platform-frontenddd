  ...
  if (isLoading) {
    return (
      <div className="animate-pulse space-y-4">
        {[1,2,3,4].map(i => <div key={i} className="h-48 bg-card rounded-2xl border border-border shadow-lg" />)}
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 bg-primary hover:brightness-110 text-white font-bold px-5 py-2.5 rounded-xl transition-colors disabled:opacity-60 shadow-lg"
        >
      ...
      {SETTING_GROUPS.map(group => (
        <div key={group.title} className="bg-card border border-border rounded-2xl p-6 shadow-lg">
