...
const StudentProfilePage = () => {
    ...
    return (
      <div className="animate-pulse space-y-4">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[1,2,3].map(i => <div key={i} className="h-24 bg-card border border-border rounded-2xl shadow-lg" />)}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Profile Card */}
      <div className="bg-card border border-border rounded-2xl p-6 flex flex-wrap items-start gap-6 shadow-lg">
        ...
        <div className="flex gap-2">
          {student.status === 'active' ? (
            <button
              ...
              className="flex items-center gap-2 border border-amber-400/50 text-amber-400 px-4 py-2 rounded-xl hover:bg-background transition-colors text-sm font-semibold"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 border border-green-400/50 text-green-400 px-4 py-2 rounded-xl hover:bg-background transition-colors text-sm font-semibold"
            >
        ...
        <div className="bg-card border border-border rounded-2xl p-6 shadow-lg">
          ...
          <div className="space-y-3">
            {student.enrollments.map((enr: any) => (
              <div key={enr.id} className="flex items-center justify-between bg-background border border-border rounded-xl p-4">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 shadow-lg">
          ...
          <div className="space-y-3">
            {student.quizAttempts.map((attempt: any) => (
              <div key={attempt.id} className="flex items-center justify-between bg-background border border-border rounded-xl p-4">
                <div>
        ...
        <div className="bg-card border border-border rounded-2xl p-6 shadow-lg">
          ...
          <div className="space-y-3">
            {student.videoProgress.map((prog: any) => (
              <div key={prog.id} className="flex items-center justify-between bg-background border border-border rounded-xl p-4">
                <div>
                ...
                <div className="text-left flex flex-col items-end gap-2">
                  ...
                  <div className="w-24 h-2 bg-background rounded-full overflow-hidden">
                    <div 
                      className={`h-full ${prog.completed ? 'bg-green-500' : 'bg-primary'}`}
                      style={{ width: `${Math.min(100, Math.max(0, prog.completion_percentage))}%` }}
                    />
                  </div>
                  {prog.completed && <span className="text-[10px] text-green-400 font-bold bg-green-500/20 px-2 py-0.5 rounded-full">مكتمل</span>}
                </div>
              </div>
            ))}
          </div>
