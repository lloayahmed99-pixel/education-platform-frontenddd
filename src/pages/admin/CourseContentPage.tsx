...
const CourseContentPage = () => {
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <div className="mt-6 border-t border-border pt-6">
          ...
          <div className="space-y-6">
            {course.modules?.map((mod: any, index: number) => (
              <div key={mod.id} className="bg-background border border-border rounded-2xl overflow-hidden shadow-lg">
                {/* Module Header */}
                <div className="bg-card px-4 py-3 flex items-center justify-between border-b border-border">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => { setActiveModule(mod); setVideoForm({ title: '', description: '', url: '', thumbnail: '', duration_seconds: 0, order_index: mod.videos?.length || 0 }); setShowVideoForm(true); }}
                      className="text-xs bg-primary text-white px-3 py-1.5 rounded-lg font-bold flex items-center gap-1 hover:brightness-110 transition-colors shadow-lg">
                    ...
                    <button onClick={() => { setActiveModule(mod); setModuleForm({ title: mod.title, order_index: mod.order_index }); setShowModuleForm(true); }}
                      className="text-blue-400 hover:text-blue-300 p-2 transition-colors"><Edit className="w-4 h-4" /></button>
                    <button onClick={() => { if(confirm('هل أنت متأكد من حذف الوحدة بجميع فيديوهاتها؟')) deleteModuleMutation.mutate(mod.id); }}
                      className="text-red-400 hover:text-red-300 p-2 transition-colors"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
                ...
                <div className="p-4 space-y-2">
                  {mod.videos?.length === 0 ? (
                    ...
                    mod.videos?.map((vid: any) => (
                      <div key={vid.id} className="flex items-center justify-between bg-card border border-border rounded-xl p-3 hover:shadow-lg transition-all">
                        <div className="flex items-center gap-3">
      ...
      {showVideoForm && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-lg">
            <div className="flex justify-between items-center mb-4">
