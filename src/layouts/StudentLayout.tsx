...
const StudentLayout = () => {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <aside className={`fixed inset-y-0 right-0 z-50 w-64 bg-card shadow-lg border-l border-border transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <nav className="p-4 space-y-2">
          ...
          <button
            onClick={logout}
            className="flex items-center w-full px-4 py-3 rounded-lg text-red-500 hover:bg-red-500/10 transition-colors"
          >
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden">
        {/* Header */}
        <header className="h-16 flex items-center justify-between px-6 bg-card shadow-lg border-b border-border">
          <div className="flex items-center lg:hidden">
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-muted hover:text-white">
              <Menu className="w-6 h-6" />
            </button>
          </div>
          <div className="flex-1"></div>
          <div className="flex items-center space-x-reverse space-x-4">
            <div className="flex items-center text-primary font-bold bg-primary/20 border border-primary/50 px-3 py-1 rounded-full">
